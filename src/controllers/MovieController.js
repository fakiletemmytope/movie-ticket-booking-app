...
const createMovie = async (req, res) => {
    ...
    const user = req.decode._id
    try {
        await db_connect()
        const movie = await movieModel.create({ title, cast, duration, genre, description, released_date, imageURL, director, user })
...
const updateMovie = async (req, res) => {
    const id = req.params.id
    const { title, cast, duration, genre, description, released_date, director } = req.body
    const update = {}
    if (title)
        update.title = title
    if (cast)
...
//adjust this
const upload_image_to_cloud = async (req, res) => {
